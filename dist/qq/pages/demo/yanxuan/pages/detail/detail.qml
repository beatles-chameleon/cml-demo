    ...
    <myscroller class=" cml-view cml-myscroller">
      <view class=" cml-base cml-view">
        <swiper class="carousel-container  cml-base cml-carousel" indicator-dots="{{true}}" current="{{1}}" circular="{{true}}">
          <swiper-item class="carousel-carousel-item  cml-base cml-carousel-item" qq:for="{{bannerImg}}" qq:for-index="i" qq:for-item="item" qq:key="i" bindtap="_cmlEventProxy" data-eventtap="{{['change']}}" data-idx="{{i}}">
            <image src="{{item.imgUrl}}" class="Img  cml-base cml-image" />
          </swiper-item>
        </swiper>
      </view>
      <view class="description  cml-base cml-view">
        <view class="description-item  cml-base cml-view" qq:for="{{descriptionInfo}}" qq:for-index="i" qq:for-item="item" data-idx="{{i}}">
